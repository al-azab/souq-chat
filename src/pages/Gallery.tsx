...
import {
  Search, Loader2, ImageIcon, Grid3X3, LayoutGrid, SlidersHorizontal,
  Calendar, Download, Trash2, X, ChevronLeft, ChevronRight, ZoomIn,
...
const GalleryPage = () => {
  ...
  const fetchItems = useCallback(async (pageNum = 0, append = false) => {
    ...
    setLoading(false);
  }, [tenantId, kindFilter, search, sortBy]);
  ...
  const handleSync = async () => {
    ...
    if (error) {
      ...
      if (data?.synced > 0) fetchItems(0, false);
    }
  };
  ...
  const navigateLightbox = (dir: 1 | -1) => {
    ...
    if (newIdx >= 0 && newIdx < items.length) {
      setLightbox({ item: items[newIdx], index: newIdx });
    }
  };
        ...
        <div className="flex border border-border rounded-lg overflow-hidden mr-auto">
          {(["lg", "md", "sm"] as const).map(size => (
            <button
              ...
              className={`p-2 transition-colors ${gridSize === size
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="bg-card rounded-xl border border-border p-12">
          <EmptyState
            icon={ImageIcon}
            title="لا توجد صور"
            description="اضغط مزامنة واتساب لجلب صور المشروعات"
          />
        </div>
      ) : (
        <>
          <div className={`grid gap-2 ${gridClasses[gridSize]}`}>
            {items.map((item, idx) => {
              ...
              return (
                <div
                  key={item.id}
                  className="group relative bg-card border border-border rounded-lg overflow-hidden cursor-pointer hover:border-primary/50 hover:shadow-md transition-all duration-200"
              ...
              <div className="flex-1 relative flex items-center justify-center p-4 min-h-0">
                {getFullUrl(lightbox.item) ? (
                  lightbox.item.kind === "video" ? (
                    <video
                      src={`https://res.cloudinary.com/${cloudName}/video/upload/${lightbox.item.storage_key}`}
                      controls
                      className="max-w-full max-h-full rounded"
                    />
                  ) : (
                    <img
                      src={getFullUrl(lightbox.item)!}
                      alt=""
              ...
              <div className="p-4 bg-card border-t border-border flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  <Badge variant="outline">
                    {kindLabel[lightbox.item.kind] || lightbox.item.kind}
                  </Badge>
                  <span className="text-sm text-muted-foreground">
                    {lightbox.item.mime || "—"} • {formatSize(lightbox.item.size_bytes)}
                  </span>
                  <span className="text-sm text-muted-foreground">
                    {formatDate(lightbox.item.received_at)}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {getFullUrl(lightbox.item) && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => window.open(getFullUrl(lightbox.item)!, "_blank")}
                      className="gap-1"
                    >
                  ...
                  <Button
                    variant="destructive"
                    size="sm"
                    onClick={() => handleDelete(lightbox.item.id)}
                    className="gap-1"
                  >
